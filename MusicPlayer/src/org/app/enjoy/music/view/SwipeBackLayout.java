...
public class SwipeBackLayout extends FrameLayout {
	...
	private int mScrollMode = LEFT_MODE;
	...
	public boolean onTouchEvent(MotionEvent event) {
		switch (event.getAction()) {
			case MotionEvent.ACTION_MOVE:
				...
				if (isSilding) {
					if (mScrollMode == RIGHT_MODE && moveX - downX >= 0) {
						mContentView.scrollBy(deltaX, 0);
					} else if (mScrollMode == LEFT_MODE && moveX - downX < 0) {
						mContentView.scrollBy(deltaX, 0);
					}
				}
				break;
			case MotionEvent.ACTION_UP:
				isSilding = false;
				if (mScrollMode == LEFT_MODE) {
					if (mContentView.getScrollX() > 0 && mContentView.getScrollX() > viewWidth / 2) {
						scrolLeft();
						isFinish = true;
					} else {
						scrollOrigin();
				...
				} else if (mScrollMode == RIGHT_MODE) {
					if (mContentView.getScrollX() < 0 && -mContentView.getScrollX() > viewWidth / 2){
						scrollRight();
						isFinish = true;
					} else {
